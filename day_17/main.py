...
StateCapture = namedtuple('StateCapture', ['rock', 'relative_heights', 'jet', ])
...
def solve():
    ...
    pattern_checker = dict()
    jet_pattern = JET_PATTERN_SAMPLE
    ...
    rocks_before_pattern = None
    heights_list = []
    for rock_no in range(HEIGHT):
        shape_index = rock_no % len(SHAPES_PATTERN)
        rock = deepcopy(rocks[shape_index])
        ...
        top_of_block_stack = max(column_heights)

        min_height = min(column_heights)
        state_heights = tuple( c - min_height for c in column_heights)
        state_capture = StateCapture(shape_index, state_heights, jet)

        if rocks_before_pattern:
            if rock_no == rocks_before_pattern:
                check_rocks = rock_no + rocks_in_cycle * cycle_repetition
                print(f"HEIGHT = {top_of_block_stack + total_height_for_repeated_section}")
                break
        elif state_capture in pattern_checker:
            print(f'Found a pattern at {rock_no} {pattern_checker[state_capture]}')
            
            rocks_in_cycle = rock_no - pattern_checker[state_capture][0]
            pattern_height = top_of_block_stack - pattern_checker[state_capture][1]
            cycle_repetition = (HEIGHT - rock_no) // rocks_in_cycle
            total_height_for_repeated_section = cycle_repetition * pattern_height

            rocks_before_pattern = rock_no + (HEIGHT - rock_no) % rocks_in_cycle
            pass
        else:
            pattern_checker[state_capture] = (rock_no, top_of_block_stack)
